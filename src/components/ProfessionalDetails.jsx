...
function Form({ onFormDataChange }) {
    ...
    const oneTextInput = (label, name, placeholder) => (
        <div className="single-row">
            <label className="input-row" htmlFor="input">
            {label}
                <input 
                    type="text"
                    id = 'input'
                    ...
                    minLength={5}
                    maxLength={100}
                    value={formState[name]} // Idk why this fixed it
    ...
    return (
        <div className="professionalInformationForm">
            <h2>Professional Experience</h2>
            <form>
                {oneTextInput("Company:", "name", "Nationwide")}
                {twoTextInput("Time:", "start", "end")}
                {oneTextInput("Location:", "location", "Columbus, OH")}
                {oneTextInput("Position:", "info", "React Developer")}
            </form>
        </div>
    )
}
